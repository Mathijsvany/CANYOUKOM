...
export type SurfaceType = 'tarmac' | 'cobbles' | 'gravel';

export function getCrrForSurface(surface: SurfaceType): number {
    switch (surface) {
        case 'cobbles': return 0.0120; // Massive penalty for cobbles
        case 'gravel': return 0.0080; // Significant penalty for gravel
        case 'tarmac':
        default: return 0.0040; // Good tarmac
    }
}
...
export function calculateEtalonWkg(
    ...
    const etalonWatts = calculateRequiredPower(
        ...
        Math.max(0.003, getCrrForSurface(surface) - 0.001) // Pro baseline is 0.003, apply relative penalty for surface
    );
...
export function calculateTargetWattsForUser(
    ...
    const crr = getCrrForSurface(surface);
